...
var position_data = {};
var searchl = {};
var gengXin_flag=true;
var lastRoutPara={time:0,
    id:0,
    old_data:0
}
get_car_position(1); 
get_car_info();
function gengXin(){
    if (gengXin_flag)  //需要停止更新，并设置为“更新”
    {
        gengXin_flag=false;
        $("#gengXin").html('更新');
    }
        ...
        $("#gengXin").html('停止更新');
        if(lastRoutPara.id) {
            update_route(lastRoutPara.id, lastRoutPara.old_data, lastRoutPara.time);
        }
    }
}
//获得车型三级联动
...
$(".tab").on('click', function () {
    ...
    if (id == "tab2") {
        show_position(2, searchl);
    }
});

function get_car_position(id) {
    $.ajax({
        url: addr + "/car/allPosition",
        type: "POST",
        data: {
            token: token,
        },
        ...
        success: function (data) {
            ...
            if (data.state == true) {
                ...
                searchl = data.data;
                show_position(id, position_data);
            } else {
                layer.msg(' ' + data.message);
            }
        },
...
function show_position(id, data) {
    // 在线地图
    var online_marker = [];
    //离线地图
    var offline_marker = [];
    if (id == 1) {
        ...
        for (var i = 0; i < data.Online.length; i++) {
            ...
            // if((data.Online)[i].longitude == "0.0"||(data.Online)[i].latitude == "0.0"){
            //     continue;
            // }
            var point = new BMap.Point((data.Online)[i].longitude, (data.Online)[i].latitude);
            online_marker[i] = new BMap.Marker(point);  // 创建标注
            map1.addOverlay(online_marker[i]);               // 将标注添加到地图中
            online_marker[i].uniqueId = (data.Online)[i].unique_id;
            online_marker[i].license = (data.Online)[i].cLicence;
            online_marker[i].wh = 0;
            online_marker[i].addEventListener("click", showInfo);
            //online_marker[i].setAnimation(BMAP_ANIMATION_BOUNCE); //跳动的动画
        }
        var data2 = toBaidupix(data.Online);
        setZoom(map1, data2);
        ...
        for (var i = 0; i < data.Offline.length; i++) {
            var point = new BMap.Point((data.Offline)[i].longitude, (data.Offline)[i].latitude);
            offline_marker[i] = new BMap.Marker(point);  // 创建标注
            map2.addOverlay(offline_marker[i]);               // 将标注添加到地图中
            offline_marker[i].uniqueId = (data.Offline)[i].unique_id;
            offline_marker[i].license = (data.Offline)[i].cLicence;
            offline_marker[i].wh = 1;
            offline_marker[i].addEventListener("click", showInfo);
            //online_marker[i].setAnimation(BMAP_ANIMATION_BOUNCE); //跳动的动画
        }
        var data2 = toBaidupix(data.Offline);
        setZoom(map2, data2);
    }
...
function showInfo(e) {
    var marker = e.target;
    car_detail(marker.uniqueId, marker.license,marker.wh);
}
...
function search() {
    ...
    if ($("input[name=search]").val() == "") {
        search_data = position_data;
    } else {
        search_data.Online = [];
        search_data.Offline = [];
        var searchText = $("input[name=search]").val();
        for (var i = 0; i < position_data.Online.length; i++) {
            if ((new RegExp(searchText)).test(position_data.Online[i].cLicence)) {
                search_data.Online.push(position_data.Online[i])
            }
        }
        for (var i = 0; i < position_data.Offline.length; i++) {
            if ((new RegExp(searchText)).test(position_data.Offline[i].cLicence)) {
                search_data.Offline.push(position_data.Offline[i])
            }
        }
    }
    searchl = search_data;
    ...
    switch (id) {
        case "tab1":
            show_position(1, search_data);
            break;
        case "tab2":
            show_position(2, search_data);
            break;
    }
}
...
function filter() {
    ...
    if (tab == "tab1") {
        $.ajax({
            url: addr + "/search/online/keywords",
            type: "GET",
            dataType: "json",
            data: {
                location: speed,
            ...
            success: function (data) {
                ...
                if (data.state == true) {
                    var lzw1 = {};
                    lzw1.Online = data.data;
                    lzw1.Offline = [];
                    show_position(1, lzw1);
                } else {
                    layer.msg(' ' + data.message);
                }
            },
        ...
        $.ajax({
            url: addr + "/search/offline/keywords",
            type: "GET",
            dataType: "json",
            data: {
                location: speed,
            ...
            success: function (data) {
                ...
                if (data.state == true) {
                    var lzw1 = {};
                    lzw1.Online =  [];
                    lzw1.Offline = data.data;
                    show_position(2, lzw1);
                } else {
                    layer.msg(' ' + data.message);
                }
            },
...
function old_route(id) {
    ...
    setTimeout(function () {
        ...
        map5.addOverlay(end_point1);
        map5.addOverlay(end_point2);
        setZoom(map5, data3);
...
function update_route(id, old_data, time) {
    lastRoutPara.id=id;
    lastRoutPara.old_data=toBaidupix(old_data);
    lastRoutPara.time=time;
    if(gengXin_flag==false) //停止更新
    {
        return;
    }
    $.ajax({
